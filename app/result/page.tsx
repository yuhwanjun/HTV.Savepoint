...
export default function ResultPage() {
  ...
  useEffect(() => {
    // Calculate result
    const classification = classifyResult(history, saveRemaining, loadRemaining, inventory);
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center overflow-hidden px-8">
      ...
      {/* Result Content */}
      <div className={`relative z-10 text-center max-w-md mx-auto transition-all duration-1000 ${isRevealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <div className="panel-angular p-4 mb-8 text-left">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="flex justify-between">
              <span className="text-[var(--color-text-muted)]">세이브 사용:</span>
              <span className="text-[var(--color-text-primary)]">{2 - saveRemaining}회</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--color-text-muted)]">로드 사용:</span>
              <span className="text-[var(--color-text-primary)]">{1 - loadRemaining}회</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--color-text-muted)]">선택 횟수:</span>
              <span className="text-[var(--color-text-primary)]">{history.length}회</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--color-text-muted)]">획득 아이템:</span>
              <span className="text-[var(--color-text-primary)]">{inventory.length}개</span>
            </div>
          </div>
        </div>
