...
// 페이즈 구조: 소개(intro) → 선택(choice) → 결과(result)
...
export interface Choice {
  ...
  label: string;
  nextNodeId: string;
  // 로드된 상태에서의 다른 결과 (optional)
  loadedNextNodeId?: string;
  // 로드된 상태에서 다른 레이블 표시 (optional)
  loadedLabel?: string;
  effect?: {
    addItem?: string;
    removeItem?: string;
  };
  // 로드된 상태에서의 다른 효과 (optional)
  loadedEffect?: {
    addItem?: string;
    removeItem?: string;
  };
...
export interface SceneNode {
  ...
  // 로드된 상태에서의 다른 확률 (optional)
  loadedChanceOutcomes?: {
    success: { probability: number; nextNodeId: string; message: string };
    failure: { probability: number; nextNodeId: string; message: string };
  };
}
...
const p1_beginning: SceneNode = {
  ...
  title: "시작의 선택",
  description:
    "당신 앞에 세 개의 무기가 놓여 있습니다. 흐릿한 기억 속에서 하나를 선택하세요.",
  allowSave: true,
  allowLoad: false,
  choices: [
    {
...
const p4_crossroads: SceneNode = {
  ...
  title: "갈림길",
  description:
    "두 갈래 길이 나타났습니다. 한쪽에서는 빛이, 다른 쪽에서는 어둠이 느껴집니다.",
  allowSave: false,
...
export const getNodeDescription = (
  ...
  const node = getNode(nodeId);
  if (!node) return "";
  if (isLoadedState && node.loadedDescription) {
    return node.loadedDescription;
  }
  return node.description;
};

// 로드 상태에 따른 선택지 정보 가져오기
export const getChoiceNextNode = (
  choice: Choice,
  isLoadedState: boolean
): string => {
  if (isLoadedState && choice.loadedNextNodeId) {
    return choice.loadedNextNodeId;
  }
...
export const getChoiceLabel = (
  ...
  if (isLoadedState && choice.loadedLabel) {
    return choice.loadedLabel;
  }
  return choice.label;
};

export const getChoiceEffect = (
  choice: Choice,
  isLoadedState: boolean
): Choice["effect"] => {
  if (isLoadedState && choice.loadedEffect) {
    return choice.loadedEffect;
  }
  return choice.effect;
};

// 로드 상태에 따른 확률 결과 가져오기
export const getChanceOutcomes = (node: SceneNode, isLoadedState: boolean) => {
  if (isLoadedState && node.loadedChanceOutcomes) {
    return node.loadedChanceOutcomes;
  }
  return node.chanceOutcomes;
};
